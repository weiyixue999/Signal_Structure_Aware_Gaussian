...
for num in $(seq 0 $max_block_id); do
    while true; do
        ...
        if [[ -n $gpu_id ]]; then
            ...
            CUDA_VISIBLE_DEVICES=$gpu_id WANDB_MODE=offline python train_large.py --config config/$CONFIG.yaml  --max_offset_k 15 --prune_outlier_iter 800 --block_id $num --port $port &
            pids+=("$!")
...
gpu_id=$(get_available_gpu)
echo "GPU $gpu_id is available."
CUDA_VISIBLE_DEVICES=$gpu_id python merge.py --config config/$CONFIG.yaml --iteration 30000
