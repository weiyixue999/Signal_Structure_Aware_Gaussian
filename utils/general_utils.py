...
def get_default_lp():
    ...
    lp.preload_images = False
    lp.image_cache_size = 0
    lp.init_point_extent_multiplier = 0.0
    lp.init_point_max_points = 0
    lp.init_scale_max_ratio = 0.0
    lp.znear = 0.01
    lp.zfar = 100.0
...
def get_default_op():
    ...
    op.densify_min_opacity = 0.005
    op.densify_max_screen_size = 20
    op.outlier_prune_start_iter = 3000
    op.outlier_prune_screen_size_scale = 30
    op.max_cache_num = 512
    op.data_loader_workers = 4
    op.pin_memory = True
    op.persistent_data_workers = True
    ...
    op.resolution_start_level = 1
    op.resolution_end_level = 5
    # Coarse training only. "dynamic" follows the scheduler, while "min"
    # keeps the coarse model at resolution_start_level for the whole run.
    op.coarse_resolution_mode = "dynamic"
    # For block training only. "min" starts from resolution_start_level,
    # "coarse" resumes from coarse_ratio.json, or set an integer level.
    op.block_resolution_start = "min"
