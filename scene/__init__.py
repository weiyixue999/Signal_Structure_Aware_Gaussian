...
def resolve_point_cloud_ply_path(pretrain_path):
    if not pretrain_path:
        return None

    if pretrain_path.endswith(".ply"):
        ply_path = pretrain_path
    elif os.path.basename(pretrain_path).startswith("iteration_"):
        ply_path = os.path.join(pretrain_path, "point_cloud.ply")
    else:
        point_cloud_dir = pretrain_path
        if os.path.basename(point_cloud_dir) != "point_cloud":
            point_cloud_dir = os.path.join(point_cloud_dir, "point_cloud")
        if not os.path.isdir(point_cloud_dir):
            raise FileNotFoundError(
                f"Pretrained point cloud directory not found: {point_cloud_dir}. "
                "Run coarse training first, or set pretrain_path to an existing "
                ".ply file, iteration_* directory, or point_cloud directory."
            )
        ...
        print(f"Resolved pretrain_path to iteration {iteration}: {ply_path}")

    if not os.path.exists(ply_path):
        raise FileNotFoundError(
            f"Pretrained point cloud not found: {ply_path}. "
            "Set pretrain_path to a .ply file, an iteration_* directory, "
            "or a point_cloud directory containing iteration_* subdirectories."
        )
    return ply_path

def prepare_initial_point_cloud(point_cloud, nerf_normalization, args):
    ...
    extent_multiplier = getattr(args, "init_point_extent_multiplier", 0.0)
    if extent_multiplier and extent_multiplier > 0:
        scene_center = -np.asarray(nerf_normalization["translate"])
        scene_radius = float(nerf_normalization["radius"])
        distances = np.linalg.norm(points - scene_center[None, :], axis=1)
        keep &= distances <= scene_radius * float(extent_multiplier)
    ...
    max_points = getattr(args, "init_point_max_points", 0)
    if max_points and max_points > 0 and points.shape[0] > max_points:
    ...
    if points.shape[0] != len(point_cloud.points):
        print(f"Prepared initial point cloud: {len(point_cloud.points)} -> {points.shape[0]} points")
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if self.loaded_iter:
            ...
            scene_info = scene_info._replace(
                point_cloud=prepare_initial_point_cloud(scene_info.point_cloud, scene_info.nerf_normalization, args)
            )
            self.gaussians.create_from_pcd(
                scene_info.point_cloud,
                self.cameras_extent,
                getattr(args, "init_scale_max_ratio", 0.0),
            )
...
class LargeScene(Scene):
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if self.loaded_iter:
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                "point_cloud",
                                                "iteration_" + str(self.loaded_iter),
                                                "point_cloud.ply"),final = True)
        elif self.pretrain_path:
            self.gaussians.load_ply(resolve_point_cloud_ply_path(self.pretrain_path))
            ...
            if args.add_background_sphere:
                ...
                background_sphere_point_rgb = np.asarray(np.random.random(background_sphere_point_xyz.shape), dtype=np.float32)
    ...
    def save(self, iteration, args=None):
        ...
        if args.block_id >= 0:
            ...
            masked_gaussians.save_ply(os.path.join(block_point_cloud_path, "point_cloud.ply"),block=True)
