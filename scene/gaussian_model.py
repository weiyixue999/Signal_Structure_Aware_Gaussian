...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, max_offset_k = 25, optimizer_type = "default"): 
    ...
    @property
    def get_offset(self):
        return torch.norm(self.get_xyz - self.anchors, dim=1, keepdim=True)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, max_initial_scale_ratio: float = 0.0):
        ...
        if max_initial_scale_ratio and max_initial_scale_ratio > 0:
            max_initial_scale = float(spatial_lr_scale) * float(max_initial_scale_ratio)
            initial_scale = torch.clamp(initial_scale, max=max_initial_scale)
            print("Clamped initial Gaussian scale to max:", max_initial_scale)
    ...
    def training_setup(self, training_args):
        ...
        if self.optimizer_type == "default":
            ...
            if SparseGaussianAdam is None:
                raise RuntimeError(
                    "optimizer_type='sparse_adam' requires the accelerated "
                    "diff_gaussian_rasterization package with SparseGaussianAdam."
                )
            self.optimizer = SparseGaussianAdam(l, lr=0.0, eps=1e-15)
        else:
            raise ValueError(
                f"Unknown optimizer_type '{self.optimizer_type}'. "
                "Use 'default' or 'sparse_adam'."
            )
    ...
    def construct_list_of_attributes(self, block = False):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
    ...
    def save_ply(self, path, block = False, final = False):
        ...
        # --- dtype list for structured array ---
        if not block and not final:
            anchors = self.anchors.cpu().numpy()
            voxel_size = self.voxel_size.cpu().numpy()
            dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes(block=False)]
        else:
            dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes(block=True)]

        elements = np.empty(N, dtype=dtype_full)

        # Compose attributes in the same order you used originally
        if (not block) and (not final):
            # we ensured anchors and voxel_size are well-shaped above
            attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, anchors, voxel_size), axis=1)
        else:
    ...
    def load_ply(self, path, final = False):
        ...
        if not final:
            anchors = np.stack((np.asarray(plydata.elements[0]["anchors_x"]),
                            np.asarray(plydata.elements[0]["anchors_y"]),
                            np.asarray(plydata.elements[0]["anchors_z"])),axis=1)
        ...
        if final:
            ...
            D = D_tensor.mean(dim=1, keepdim=True)
            self.voxel_size = D.float().cuda()
            # voxel_size = np.asarray(plydata.elements[0]["voxel_size"])[...,np.newaxis]
            # self.anchors = torch.tensor(anchors).float().cuda()
