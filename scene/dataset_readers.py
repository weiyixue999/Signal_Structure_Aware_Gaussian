...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              ...
                              depth_param = depth_param, depth_filepath=depth_filepath, intrinsic=K)
...
def readColmapSceneInfo(path, images, eval, llffhold=None, partition=None):
    ...
    try:
        ...
        cameras_extrinsic_file_eval = os.path.join(path2, "sparse/0", "images.bin")
        cameras_intrinsic_file_eval = os.path.join(path2, "sparse/0", "cameras.bin")
        cam_extrinsics_eval = read_extrinsics_binary(cameras_extrinsic_file_eval)
        cam_intrinsics_eval = read_intrinsics_binary(cameras_intrinsic_file_eval)
    except:
    ...
    cam_infos_unsorted_eval = readColmapCameras(cam_extrinsics=cam_extrinsics_eval, cam_intrinsics = cam_intrinsics_eval,
                                                 images_folder = os.path.join(path2, reading_dir))   
    cam_infos_eval = sorted(cam_infos_unsorted_eval.copy(), key = lambda x: x.image_name)
    test_cam_infos = cam_infos_eval
